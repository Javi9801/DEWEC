...
function mostrar(){
    ...
    const rpersona = document.getElementById('btnpersona');
    const rempresa = document.getElementById('btnempresa');
    
    // evento para el input radio del "persona"
    rpersona.onclick = function() {
...
function validar(dni,fecha,fechaCarnet,telefono){
    ...
    valida.onclick = function(){
        ...
        var fcarnet = fechaCarnet.value;
        var tel = telefono.value;

        if(dniV == "" || fechanac=="" || fcarnet=="" || tel==""){
            alert("LOS CAMPOS DNI, FECHA NACIMIENTO, FECHA CARNET NO PUEDEN SER NULOS");
        } else {
